...
export default class HotUpdate extends cc.Component {
    ...
    /** 设置验证方法 */
    setVerifycb(path: string, asset: jsb.ManifestAsset) {
    ...
    initAssetsManager(url?: string) {
        if (this.am.getState() === jsb.AssetsManager.State.UNINITED) {
            if (!url) url = this.mainifestUrl.nativeUrl;
    ...
    checkUpdate() {
        ...
        cc.log("原包版本信息url:", url);
        this.initAssetsManager(url);
    ...
    */
    _modifyAppLoadUrlForManifestFile(newAppHotUpdateUrl: string, localManifestPath: string) {
        let isWritten = false;
